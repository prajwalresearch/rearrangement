...
class DeepLagrangianNetwork(nn.Module):
    ...
    def __init__(self, q_dim=14, hidden_dim=64, device="cpu", ndim=6):
        super().__init__()
        self.q_dim = q_dim
    ...
    def assemble_lower_triangular_matrix(self, Lo, Ld):
        ...
        L = torch.tril(torch.ones(*diagonal_matrix.shape, device=self.device)) - torch.eye(self.q_dim_changed,device=self.device) # gives lower triangle matrix. 
    ...
    def forward(self, x):
        ...
        H = L @ L.transpose(1, 2) + 1e-9 * torch.eye(int(self.ndim/2), device=self.device)
        return H
